    ...
    defStyleAttr: Int = 0) : BulletGraph(context, attrs, defStyleAttr)
{
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.drawText(labelB?.get(0).toString(), graphMargin, top - labelSize, labelStartPaint)

        canvas?.drawText(labelA?.get(number).toString(), (number - 1) * ratio + graphMargin + (ratio - getTextWidth(labelPaint, labelA?.get(number).toString(), boundRect)) / 2, LabelABottom, labelPaint)
        canvas?.drawText(labelB?.get(number).toString(), number * ratio + graphMargin, top - labelSize, labelEndPaint)

        for (i in 1 until number) {
            canvas?.drawText(
                labelA?.get(i).toString(),
                (i - 1) * ratio + graphMargin + (ratio - getTextWidth(labelPaint, labelA?.get(i).toString(), boundRect)) / 2,
                LabelABottom,
                labelPaint)
            canvas?.drawText(
